...
def run_algorithm(algo, sup, maxedge, filename, outputfile, superverbose = False, verbose = True,directed = True, edge_removal = False, projection = 'full', edge_color = False, node_color = False):
    '''
    ...
    if algo in ['germ','Germ','GERM']:
        ...
        if superverbose:
            command = f"./algorithms/germ {sup} {filename} {maxedge}"
        if verbose:
        ...
        if superverbose:
            command = f"./algorithms/evomine -s {sup} -e {maxedge} -T {projection} {params} -f {filename}"
        if verbose:
            print("Running command:")
            print(command)
        os.system(command)
